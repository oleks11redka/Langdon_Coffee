...
export default class LocationItem extends Component {
    render() {
      ...
      return (
        <div className='locationItem' style={{ height: `${phone === '' ? '140px' : null }` }}>
          <h3 className='locationItem__title'>{name}</h3>
          <p className='locationItem__phone' style={{ display: `${phone === '' ? 'none' : null }` }}>{phone ? <p><span className='locationItem__phone--margin'>Phone:</span>{phone}</p> : null}</p>
          <p className='locationItem__adress'><span className='locationItem__adress--margin'>Address:</span> {address}</p>
